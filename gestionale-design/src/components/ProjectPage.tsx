...
const TIMELINE_EVENTS = [
  { id: 1, type: 'issue', user: 'Marco Rossi', avatar: 'https://i.pravatar.cc/150?u=1', time: '10:45', content: 'Tubo rotto nel settore B. Necessario intervento idraulico urgente.', status: 'Aperto' },
  { id: 2, type: 'hours', user: 'Luca Verdi', avatar: 'https://i.pravatar.cc/150?u=3', time: '09:30', content: 'Loggate 4h - Getto calcestruzzo pilastro A4' },
  { id: 3, type: 'message', user: 'Giulia Neri', avatar: 'https://i.pravatar.cc/150?u=4', time: 'Ieri, 16:20', content: 'I materiali per la copertura sono arrivati in cantiere. Iniziamo lo scarico.' },
...
export default function ProjectPage() {
  ...
  const handleShare = (item: any) => {
    setItemToShare({
      title: item.content || item.title || 'Elemento',
        ...
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <MetricCard title="Avanzamento" value="65%" sub="In linea con i tempi" trend="positive" />
              <MetricCard title="Ore Lavorate" value="340 / 500" sub="160 ore rimanenti" trend="neutral" />
              <MetricCard title="Problemi Aperti" value="2" sub="1 critico" trend="negative" />
            </div>
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-3">
              <h2 className="text-3xl font-bold text-text-primary tracking-tight">Cantiere Milano Nord</h2>
              <span className="px-3 py-1 bg-info-bg text-info-text text-xs font-bold rounded-lg uppercase tracking-wider border border-info-border">In Corso</span>
            </div>
            <div className="flex flex-wrap items-center gap-6 text-sm text-text-secondary font-medium">
              <div className="flex items-center gap-2"><Building2 size={16} className="text-text-secondary opacity-60" /> Cliente: Immobiliare SPA</div>
              <div className="flex items-center gap-2"><Users size={16} className="text-text-secondary opacity-60" /> Squadra A (8 persone)</div>
              <div className="flex items-center gap-2"><Calendar size={16} className="text-text-secondary opacity-60" /> Scadenza: 15 Mag 2024</div>
            </div>
          </div>
