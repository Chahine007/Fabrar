...
export function usePricebook() {
  return useQuery({
    ...
    queryFn: () => fetchJson<PricebookItem[]>('/api/pricebook'),
...
export function useCantiereMaterials(cantiereId: number | null) {
  return useQuery({
    ...
    queryFn: () => fetchJson<ProjectMaterialRow[]>(`/api/cantieri/${cantiereId}/materials`),
...
export function useAddMaterialToCantiere(cantiereId: number) {
  ...
  return useMutation({
    mutationFn: async (payload: AddProjectMaterialPayload) => {
      const body = {
      ...
      return res.json() as Promise<ProjectMaterialRow>;
    },
    onSuccess: () => {
